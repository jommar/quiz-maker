...
const QuizTaker = ({ quizIndex, onBack }) => {
  ...
  if (showResult) {
    return <QuizResult quiz={{ ...quiz, questions }} selectedAnswers={selectedAnswers} onBack={onBack} />;
  }
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        {currentQuestion.options.map((opt, idx) => {
          ...
          let backgroundColor = '';
          if (isSelected) {
            if (quiz.config.showCorrectAnswersWhileAnswering && currentAnswer && !isCorrect) {
              backgroundColor = isCorrectAnswer ? '#4caf50' : '#f44336'; // Green correct, Red wrong
            } else {
              backgroundColor = '#2196f3'; // Blue selected
            }
          } else if (quiz.config.showCorrectAnswersWhileAnswering && currentAnswer && !isCorrect && isCorrectAnswer) {
        ...
        {currentAnswer && !isCorrect && quiz.config.showCorrectAnswersWhileAnswering && (
          <div style={{
            marginTop: '10px',
            padding: '10px',
            ...
            Wrong Answer! Correct Answer: <strong>{currentQuestion.options[currentQuestion.answer]}</strong>
          </div>
        )}
      </div>
